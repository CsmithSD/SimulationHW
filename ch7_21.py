...
'''
1D algorithm from book:

procedure[x] = randmindpoint1d( nrc, sigma, seed )
	initialize_randn( seed )
	N <- pow( 2, nrc )
	x(0) <- 0
	x(N) <- sigma*randn
	for i from 1 to nrc:
...
procedure Recursion( x, t0, t2, t, nrc )
	t1 <- ( t0 + t2 )/2
	x(t1) <- 0.5*( x(t0) + x(t2) ) + delta(t)*randn
	if t < nrc
		Recursion( x, t0, t1, t+1, nrc )
		Recrusion( x, t1, t2, t+1, nrc )
		
nrc - number of recursion
sigma - initial standard deviation of gaussian distribution randn
delta - vector of variances for each point
x - vector containing brownian motion on 1D
H - 0 < H < 1 - roughness of the fraction at small scales
